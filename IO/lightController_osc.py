...
def main():
    ...
    while running and not shutdown_requested:
        ...
        if ws_broadcaster and (not hasattr(ws_broadcaster, 'last_broadcast') or 
            ...
            try:
                ...
                state = {
                    ...
                    # V6.1d: Read from V6 SmartAutoTuner instead of disabled V5 auto_tuner
                    'auto_tuning': (lambda _at=v6.autotuner if v6 else None: {
                        'enabled': _at.enabled if _at else False,
                        'revision': len(_at.adjustment_history) if _at else 0,
                        'params': {name: round(float(getattr(meta_params, name)), 3) for name in auto_tuner.param_order},
                        'activity': {
                            'score': round(_at.adjustment_history[-1].get('score', 0), 3) if (_at and _at.adjustment_history) else 0,
                            'regime': _at.adjustment_history[-1].get('regime', '') if (_at and _at.adjustment_history) else '',
                        } if (_at and _at.adjustment_history) else None,
                        'budget': {
                            'current': round(_at.budget, 1) if _at else 0,
                            'max': round(_at.config.budget_max, 1) if _at else 200,
                        },
                        'top_deltas': [
                            {'name': n, 'delta': round(d, 4)}
                            for n, d in sorted(
                                (_at.adjustment_history[-1].get('deltas', {}) if (_at and _at.adjustment_history) else {}).items(),
                                key=lambda kv: abs(kv[1]), reverse=True
                            )[:4]
                        ] if (_at and _at.adjustment_history) else [],
                    })(),
                    'population': {
        ...
        # V6.1d: Read from V6 SmartAutoTuner instead of disabled V5 auto_tuner
        v6_at = v6.autotuner if v6 else None
        draw_auto_tuning_panel(
            ...
            font,
            font_small,
            v6_at.enabled if v6_at else False,
            v6_at.adjustment_history[-1] if (v6_at and v6_at.adjustment_history) else None,
            v6_at.adjustment_history if v6_at else [],
            v6_at.budget if v6_at else 0,
            v6_at.config.budget_max if v6_at else 200,
        )

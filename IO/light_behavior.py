...
class GestureType(Enum):
    ...
    # V5: New gestures using anisotropic falloff
    SWEEP = "sweep"              # Wide X scan - falloff stretches horizontally + slow X translation
    FOCUS = "focus"              # Spotlight narrowing - falloff contracts + brightens
...
class BehaviorState:
    ...
    gesture_brightness_boost: float = 0.0              # extra brightness from FOCUS gesture
    
    # V5: Dwell phase tracking for transition pulses
    last_dwell_phase: str = 'none'                     # Previous dwell phase for transition detection
...
class BehaviorSystem:
    ...
    TRANSITIONS = {
        (BehaviorMode.IDLE, BehaviorMode.ENGAGED): 0.4,    # V5: snappier (was 0.8)
    ...
    ENTRY_PULSE_BRIGHTNESS_BOOST = 50  # V5: stronger pulse (was 25)
    ENTRY_PULSE_DURATION = 1.5         # V5: longer visibility (was 0.8)
    
    # V5: Re-entry pulse (acknowledgment when new person enters during ENGAGED/CROWD)
    REENTRY_PULSE_BRIGHTNESS_BOOST = 30
    REENTRY_PULSE_DURATION = 0.5
    
    # V5: Phase transition pulse (subtle flash when dwell phase advances)
    PHASE_PULSE_BRIGHTNESS_BOOST = 10
    PHASE_PULSE_DURATION = 0.3
    ...
    STATUS_TEXTS = {
        ...
        ('engaged', 'sweep'): [
            "Sweeping across. Dramatic gesture.",
            "Wide beam sweep. Showing off.",
            "Stretching light across the space.",
        ],
        ('engaged', 'focus'): [
            "Focusing beam. Intense attention.",
            "Narrowing in. Deep concentration.",
            "Tight spotlight. All attention here.",
        ],
        ('crowd', 'default'): [
    ...
    ENGAGED_GESTURE_CONFIG = {
        ...
        'greet': {
            'interval_range': (6, 12),     # V5: slightly more frequent (was 8-15)
            'gestures': [
                ...
                (GestureType.SWEEP, 2.5, 2),       # V5: wide scan gesture
            ],
        },
        'engage': {
            'interval_range': (8, 16),    # V5: slightly more frequent (was 10-20)
            'gestures': [
                ...
                (GestureType.SWEEP, 3.0, 2),        # V5: wide scan
                (GestureType.FOCUS, 1.5, 2),        # V5: spotlight narrowing
            ],
        },
        'bond': {
            'interval_range': (12, 25),    # V5: slightly more frequent (was 15-30)
            'gestures': [
                ...
                (GestureType.FOCUS, 2.0, 2),         # V5: focused spotlight
                (GestureType.SWEEP, 3.5, 1),         # V5: occasional wide scan
            ],
        },
    }
    
    # Amplitude settings for engaged gestures (in cm)
    # V5: Added falloff shape modifiers (scale_x/y/z, rotation) to existing gestures
    ENGAGED_GESTURE_AMPLITUDES = {
        GestureType.NOD:    {'y': 12},                  # Small Y bob
        GestureType.LEAN:   {'z': 15, 'scale_z': 1.5},  # V5: Forward Z shift + depth stretch
        GestureType.SWAY:   {'x': 18, 'scale_x': 1.8},  # V5: Lateral X + horizontal stretch
        GestureType.ORBIT:  {'x': 15, 'y': 8, 'rotation': 0.4},  # V5: XY circle + rotation
        GestureType.SETTLE: {'z': -8, 'radius': -10, 'scale_x': 0.7, 'scale_z': 0.7},  # V5: focused spot
        GestureType.SWEEP:  {'x': 25, 'scale_x': 2.5},  # V5: wide X scan with stretched falloff
        GestureType.FOCUS:  {'scale_x': 0.5, 'scale_z': 0.5, 'brightness_boost': 20},  # V5: spotlight narrow
    }
    ...
    def _compute_engaged_gesture_position(self) -> Optional[np.ndarray]:
        ...
        V5: Also computes falloff shape parameters (scale_x/y/z, rotation)
        stored in self.state for pickup by calculate_parameters().
        ...
        # V5: Compute falloff shape modifiers from gesture amplitudes
        # These are picked up by calculate_parameters() and sent to the controller
        gesture_scale_x = 1.0 + (amps.get('scale_x', 1.0) - 1.0) * wave
        gesture_scale_y = 1.0 + (amps.get('scale_y', 1.0) - 1.0) * wave
        gesture_scale_z = 1.0 + (amps.get('scale_z', 1.0) - 1.0) * wave
        gesture_rotation = amps.get('rotation', 0.0) * wave
        
        # Store for pickup by calculate_parameters  
        self.state.gesture_falloff_scale = (gesture_scale_x, gesture_scale_y, gesture_scale_z)
        self.state.gesture_falloff_rotation = gesture_rotation
        self.state.gesture_brightness_boost = amps.get('brightness_boost', 0) * wave
        ...
        elif gesture == GestureType.SWEEP:
            # V5: Wide X scan — slow translation with stretched falloff
            # Smooth left-to-right sweep (half cycle to avoid returning)
            sweep_wave = math.sin(t * math.pi)  # Bell shape
            base[0] += amps.get('x', 25) * (math.sin(t * math.pi * 2) * 0.5) * sweep_wave
        
        elif gesture == GestureType.FOCUS:
            # V5: Spotlight narrowing — no position change, just falloff contraction
            # Position stays at target; brightness and shape handled via state vars
            pass
    ...
    def trigger_entry_pulse(self, reentry=False):
        ...
        V5: If reentry=True, use smaller REENTRY_PULSE settings.
        """
        self.state.entry_pulse_active = True
        self.state.entry_pulse_start = time.time()
        if reentry:
            self.state.entry_pulse_duration = self.REENTRY_PULSE_DURATION
            # Store which boost to use — _update_entry_pulse reads ENTRY_PULSE_BRIGHTNESS_BOOST
            # but calculate_parameters() will use this override
            self._active_pulse_boost = self.REENTRY_PULSE_BRIGHTNESS_BOOST
        else:
            self.state.entry_pulse_duration = self.ENTRY_PULSE_DURATION
            self._active_pulse_boost = self.ENTRY_PULSE_BRIGHTNESS_BOOST
    ...
    def on_person_entered(self, person_id: int, position: np.ndarray, is_active_zone: bool):
        ...
        if is_active_zone and self.meta.entrance_flash_enabled:
            # V5: Use re-entry pulse if already engaged/crowd with existing people
            already_engaged = (self.state.mode in (BehaviorMode.ENGAGED, BehaviorMode.CROWD) 
                              and len(self.active_zone_people) > 0)
            self.trigger_entry_pulse(reentry=already_engaged)
    ...
    def apply_dwell_rewards(self, params: Dict) -> Dict:
        ...
        V5: Stronger phase bonuses, phase transition pulses, 
        and falloff shape modulation per phase.
        ...
        if stats['count'] == 0:
            self.state.current_dwell_bonus = 0
            self.state.last_dwell_phase = 'none'
            return params
        ...
        # --- V5: Phase transition detection & pulse ---
        if phase != self.state.last_dwell_phase and self.state.last_dwell_phase != 'none':
            # Phase just changed — fire a small transition pulse
            now = time.time()
            self.state.entry_pulse_start = now
            self.state.entry_pulse_active = True
            self.state.entry_pulse_duration = self.PHASE_PULSE_DURATION
            self._active_pulse_boost = self.PHASE_PULSE_BRIGHTNESS_BOOST
        self.state.last_dwell_phase = phase
        ...
        if phase == 'notice':  # 0-3s
            ...
            # Greeting - settling in  (V5: +15% base brightness via stronger bonus)
            brightness_add = 8 * influence    # was 5
            interval_add = 1 * influence
            result['brightness_max'] = result.get('brightness_max', 55) + brightness_add
            result['wander_interval'] = result.get('wander_interval', 4.0) + interval_add
            self.state.current_dwell_bonus = 8 * influence
            
        elif phase == 'engage':  # 10-30s
            # Active engagement  (V5: stronger ramp, up to +20)
            base_bonus = min(20, int((dwell_time - 10) / 4) * 3)  # +3 per 4s, max +20
            bonus = base_bonus * influence
            result['brightness_max'] = result.get('brightness_max', 55) + bonus
            result['brightness_min'] = result.get('brightness_min', 12) + bonus * 0.5
            speed_mult = 1.0 - 0.25 * influence  # 0.75 at influence=1
            result['move_speed'] = result.get('move_speed', 40) * max(0.4, speed_mult)
            result['wander_interval'] = result.get('wander_interval', 4.0) + 2.5 * influence
            self.state.current_dwell_bonus = bonus
            
        elif phase == 'bond':  # 30s+
            # Deep bond - maximum intimacy  (V5: +25 brightness)
            bonus = 25 * influence    # was 15
            result['brightness_max'] = result.get('brightness_max', 55) + bonus
            result['brightness_min'] = result.get('brightness_min', 12) + 12 * influence
            speed_mult = 1.0 - 0.4 * influence  # 0.6 at influence=1
            result['move_speed'] = result.get('move_speed', 40) * max(0.3, speed_mult)
            ...
            self.state.current_dwell_bonus = 25
        
        # Multi-person adjustments
        if count >= 2:
            # With 2+ people, be slightly more energetic
            result['move_speed'] = result.get('move_speed', 40) * (1 + 0.1 * min(count, 4))
            result['brightness_max'] = result.get('brightness_max', 55) * (1 + 0.05 * count)
    ...
    def calculate_parameters(self, active_count: int, passive_count: int,
        ...
        # V5: Use _active_pulse_boost which varies per entry/re-entry/phase
        entry_pulse_intensity = self._update_entry_pulse(now)
        pulse_boost = getattr(self, '_active_pulse_boost', self.ENTRY_PULSE_BRIGHTNESS_BOOST)
        if entry_pulse_intensity > 0:
            params['brightness_max'] = params.get('brightness_max', 55) + pulse_boost * entry_pulse_intensity
            params['brightness_min'] = params.get('brightness_min', 12) + pulse_boost * 0.5 * entry_pulse_intensity
            ...
            # V5: Contract falloff shape during entry pulse for focused beam
            pulse_contract = 1.0 - 0.4 * entry_pulse_intensity  # squeeze to 60%
            params['falloff_scale_x'] *= pulse_contract
            params['falloff_scale_z'] *= pulse_contract
        ...
        if self.state.bloom_active or self.state.bloom_progress > 0:
            ...
            # V5: Neutralize falloff shape during bloom (uniform spread)
            bp = self.state.bloom_progress
            params['falloff_scale_x'] = params['falloff_scale_x'] * (1 - bp) + 1.0 * bp
    ...
    def update_status_text(self, active_count: int):
        ...
        if self.state.bloom_active:
            ...
            if self.state.gesture == GestureType.NOD:
                ...
                key = ('engaged', 'orbit')
            elif self.state.gesture == GestureType.SWEEP:
                key = ('engaged', 'sweep')
            elif self.state.gesture == GestureType.FOCUS:
                key = ('engaged', 'focus')

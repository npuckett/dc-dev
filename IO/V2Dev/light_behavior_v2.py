...
class BehaviorSystem:
    ...
    def update_idle_trends(self) -> IdleTrends:
        """
        Query database and compute current idle trends.
        Called periodically when in IDLE mode (every 5-10 seconds).
        """
        now = time.time()
        ...
        try:
            ...
            # Note: These are cumulative counts within the time window
            recent_stats = self.database.get_current_stats()
            if recent_stats:
                trends.recent_passive_count = recent_stats.get('passive_events', 0)
                trends.recent_active_count = recent_stats.get('active_events', 0)
                trends.recent_avg_speed = recent_stats.get('avg_speed', 0) or 0.0
                ltr = recent_stats.get('flow_left_to_right', 0) or 0
                rtl = recent_stats.get('flow_right_to_left', 0) or 0
                if ltr + rtl > 0:
            ...
            short_trends = self.database.get_trends(5)
            if short_trends:
                trends.short_passive_count = short_trends.passive_zone_count
                trends.short_active_count = short_trends.active_zone_visits
                if short_trends.left_to_right_count + short_trends.right_to_left_count > 0:
                    trends.short_flow_direction = (
                        (short_trends.left_to_right_count - short_trends.right_to_left_count) /
                        (short_trends.left_to_right_count + short_trends.right_to_left_count)
                    )
                # Activity level based on total events (passive + active)
                total_short = short_trends.passive_zone_count + short_trends.active_zone_visits
                trends.short_activity_level = min(1.0, total_short / 50.0)
                trends.has_short_data = (total_short > 0)
            
            # Medium term (30 minutes)
            medium_trends = self.database.get_trends(30)
            if medium_trends:
                trends.medium_passive_count = medium_trends.passive_zone_count
                trends.medium_active_count = medium_trends.active_zone_visits
                if medium_trends.left_to_right_count + medium_trends.right_to_left_count > 0:
                    trends.medium_flow_direction = (
                        (medium_trends.left_to_right_count - medium_trends.right_to_left_count) /
                        (medium_trends.left_to_right_count + medium_trends.right_to_left_count)
                    )
                total_medium = medium_trends.passive_zone_count + medium_trends.active_zone_visits
                trends.medium_activity_level = min(1.0, total_medium / 200.0)
                trends.has_medium_data = (total_medium > 0)
            
            # Long term (1 hour)
            long_trends = self.database.get_trends(60)
            if long_trends:
                trends.long_passive_count = long_trends.passive_zone_count
                trends.long_active_count = long_trends.active_zone_visits
                if long_trends.left_to_right_count + long_trends.right_to_left_count > 0:
                    trends.long_flow_direction = (
                        (long_trends.left_to_right_count - long_trends.right_to_left_count) /
                        (long_trends.left_to_right_count + long_trends.right_to_left_count)
                    )
                total_long = long_trends.passive_zone_count + long_trends.active_zone_visits
                trends.long_activity_level = min(1.0, total_long / 400.0)
                trends.has_long_data = (total_long > 0)
            
            # Historical pattern for this time period
            hourly_pattern = self.database.get_hourly_pattern(7)  # Last 7 days
            current_hour = datetime.now().hour
            if hourly_pattern and current_hour in hourly_pattern:
                hp = hourly_pattern[current_hour]
                trends.period_typical_count = hp.get('avg_people', 0)
                trends.period_typical_flow = hp.get('flow_balance', 0.0)
    ...
    def update_flow(self, dt: float):
        ...
        try:
            # Query database for 30-second window
            # Use a direct query since get_trends uses minute-based windows
            with self.database.lock:
                ...
                if row:
                    flow.left_to_right_count = row['ltr'] or 0
                    flow.right_to_left_count = row['rtl'] or 0
                    flow.total_events = row['total'] or 0
        except Exception as e:
            # Database error, keep previous values
            return
        ...
        # Strength is high when there's consistent directional flow
        if flow.total_events >= 3:  # Need at least 3 events for confidence
            flow.strength = min(1.0, abs(raw_direction) * (flow.total_events / 10.0))
        else:
            flow.strength = 0.0  # Not enough data
        ...
        # The light should position toward where people are coming FROM
        # If flow is left-to-right (positive), people come from left (negative X)
        # So we shift the box to the LEFT (more negative X) to "greet" incoming people
        #
        # X coordinate system: -290 to -30 (more negative = further along the panels)
        # Shift up to 60cm based on flow direction and strength
        max_shift = 60.0  # Maximum shift in centimeters
        
        # Only apply shift when strength is significant
        if flow.strength > 0.2:
            # Negative shift = move box left (toward negative X)
            # Positive flow = people coming from left = shift box left to greet them
            flow.x_offset = -flow.direction * flow.strength * max_shift
        else:
            # Gradually return to center when no clear flow
            flow.x_offset *= 0.9  # Decay toward zero

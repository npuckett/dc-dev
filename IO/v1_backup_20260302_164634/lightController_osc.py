...
- New gestures: SWEEP (wide X scan) and FOCUS (spotlight narrowing)
- Faster/clearer ENGAGED mode transitions and stronger entry pulse
- Higher max intensity (up to 100) for impact moments
- Time-of-day home value profiles for autotuning (replaces periodic resets)
- Reduced mean reversion on output multipliers for more expressive range
- Enhanced dwell progression with phase-transition pulses
- Re-entry acknowledgment pulses in ENGAGED/CROWD mode

Based on lightController_osc.py (V3)
...
# Add parent directory (IO/) to path for shared imports (tracking_database, etc.)
# Also add this directory for v5-specific imports
V5_DIR = os.path.dirname(os.path.abspath(__file__))
IO_DIR = os.path.dirname(V5_DIR)
sys.path.insert(0, V5_DIR)
sys.path.insert(0, IO_DIR)

# Tracking database (shared from IO/)
from tracking_database import TrackingDatabase

# Behavior system (V5 version with anisotropic falloff + new gestures)
...
# Resolve all relative paths to IO/ directory (parent of v5Dev/)
# This ensures the DB, slider settings, and reports are in the same place as production
...
class AutoTuningManager:
    def __init__(self, meta: MetaParameters, sliders: dict, database: TrackingDatabase = None):
        ...
        # V5: Static home_values replaced by time-of-day profiles.
        # These serve as the default / fallback.
        self.home_values = {
            'responsiveness': 0.50,
            'energy': 0.45,
            'attention_span': 0.50,
        ...
        # V5: Time-of-day profiles — home values shift smoothly throughout the day.
        # Keys are hours (0-23). Values between defined hours are linearly interpolated.
        # This replaces the periodic 6-hour reset blends with continuous variation.
        # "Morning" = energetic/exploratory, "Afternoon" = sociable/engaged,
        # "Evening" = bright/dramatic, "Night" = quiet/contemplative
        self._time_profiles = {
            0: {  # Midnight — quiet, contemplative
                'brightness_global': 0.8,
                'speed_global': 0.50,
                'pulse_global': 0.60,
                'energy': 0.30,
                'responsiveness': 0.40,
                ...
                'exploration': 0.50,
                'attention_span': 0.60,
                'dwell_influence': 0.40,
            },
            6: {  # Early morning — waking up, exploratory
                'brightness_global': 1.0,
                'speed_global': 0.65,
                'pulse_global': 0.70,
                'energy': 0.45,
                'responsiveness': 0.50,
                ...
                'exploration': 0.55,
                'attention_span': 0.50,
                'dwell_influence': 0.45,
            },
            10: {  # Mid-morning — active, energetic
                'brightness_global': 1.2,
                'speed_global': 0.75,
                'pulse_global': 0.85,
                'energy': 0.55,
                ...
                'exploration': 0.45,
                'attention_span': 0.45,
                'dwell_influence': 0.55,
            },
            14: {  # Afternoon — sociable, engaged
                'brightness_global': 1.3,
                'speed_global': 0.80,
                'pulse_global': 0.90,
                'energy': 0.55,
                'responsiveness': 0.60,
                ...
                'exploration': 0.35,
                'attention_span': 0.45,
                'dwell_influence': 0.60,
            },
            18: {  # Evening — bright, dramatic, high engagement
                'brightness_global': 1.5,
                'speed_global': 0.85,
                'pulse_global': 1.00,
                'energy': 0.60,
                'responsiveness': 0.60,
                ...
                'exploration': 0.40,
                'attention_span': 0.50,
                'dwell_influence': 0.65,
            },
            22: {  # Late evening — winding down
                'brightness_global': 1.1,
                'speed_global': 0.60,
                'pulse_global': 0.75,
                'energy': 0.40,
                'responsiveness': 0.45,
        ...
        # Sort profile hours for interpolation
        self._time_profile_hours = sorted(self._time_profiles.keys())
        ...
        # V5: Removed periodic 6-hour resets — replaced by continuous time-of-day
        # home value interpolation via _get_current_home_values()
        
        # V5: Reduced mean reversion to let parameters drift further from home
        # Output multipliers (brightness, speed, pulse) get half reversion
        self._reversion_base = 0.01       # was 0.02
        self._reversion_progressive = 0.03  # was 0.06
        self._output_reversion_scale = 0.5  # Halve reversion for brightness/speed/pulse
    ...
    def _get_current_home_values(self) -> dict:
        """V5: Get home values interpolated by current time of day.
        
        Smoothly blends between the nearest defined time profile hours.
        For params not in a profile, falls back to self.home_values defaults.
        """
        now = datetime.now()
        current_hour = now.hour + now.minute / 60.0  # fractional hour
        
        hours = self._time_profile_hours
        if not hours:
            return dict(self.home_values)
        
        # Find surrounding profile hours for interpolation
        lower_hour = hours[-1]  # wrap around (e.g., 22 for midnight)
        upper_hour = hours[0]   # wrap around (e.g., 0 for midnight)
        for i, h in enumerate(hours):
            if h <= current_hour:
                lower_hour = h
                upper_hour = hours[(i + 1) % len(hours)]
            else:
                upper_hour = h
                break
        ...
        if lower_hour == upper_hour:
            ...
            span = upper_hour - lower_hour
            if span <= 0:
                span += 24  # wrap around midnight
            elapsed = current_hour - lower_hour
            if elapsed < 0:
                elapsed += 24
            t = elapsed / span
        
        lower_profile = self._time_profiles.get(lower_hour, {})
        upper_profile = self._time_profiles.get(upper_hour, {})
        
        # Start with defaults, then override with interpolated profile values
        result = dict(self.home_values)
        all_keys = set(lower_profile.keys()) | set(upper_profile.keys())
        for key in all_keys:
            low_val = lower_profile.get(key, self.home_values.get(key, 0.5))
            high_val = upper_profile.get(key, self.home_values.get(key, 0.5))
            result[key] = low_val * (1.0 - t) + high_val * t
    ...
    def update(self, behavior_status: dict, now: float) -> Optional[dict]:
        ...
        # --- V5: CONTINUOUS TIME-OF-DAY HOME VALUES ---
        # Instead of periodic 6-hour reset blends, home values continuously
        # shift throughout the day via interpolated time profiles.
        time_home_values = self._get_current_home_values()
        ...
        # --- V5: REDUCED PROGRESSIVE MEAN REVERSION WITH TIME-OF-DAY HOMES ---
        # Uses interpolated time-of-day home values instead of static ones.
        # Reduced base/progressive to let params drift further from home.
        # Output multipliers (brightness, speed, pulse) get half reversion.
        ...
        for name, home_val in time_home_values.items():
            ...
            if current_val is not None:
                ...
                # V5: Halve reversion for output multipliers
                if name in output_params:
                    strength *= self._output_reversion_scale
                pull = distance_from_home * strength
                deltas[name] = deltas.get(name, 0.0) + pull

        # --- FIX 5: CURIOSITY BIASED TOWARD TIME-OF-DAY HOME ---
        # V5: Uses time-interpolated home values for curiosity bias direction
        if now - self._last_curiosity_time > self._curiosity_interval:
            ...
            for name in self.param_order:
                ...
                home_val = time_home_values.get(name)
                current_val = current_values.get(name)
...
@dataclass
class PointLight:
    """Virtual point light with anisotropic falloff support.
    ...
    brightness_max: int = 100  # V5: raised from 40, most visible range still 1-50
    pulse_speed: float = 2000
    falloff_radius: float = 50
    
    # V5: Anisotropic falloff shape
    falloff_scale: np.ndarray = field(default_factory=lambda: np.array([1.0, 1.0, 1.0]))
    target_falloff_scale: np.ndarray = field(default_factory=lambda: np.array([1.0, 1.0, 1.0]))
    falloff_rotation: float = 0.0       # Current rotation (radians, around Y axis)
    ...
    def update(self, dt: float):
        ...
        # V5: Animate falloff scale toward target with spring dynamics
        scale_diff = self.target_falloff_scale - self.falloff_scale
        scale_factor = 1.0 - math.exp(-self.scale_spring_speed * dt)
        self.falloff_scale = self.falloff_scale + scale_diff * scale_factor
        
        # V5: Animate falloff rotation toward target
        rot_diff = self.target_falloff_rotation - self.falloff_rotation
...
class PanelSystem:
    ...
    def calculate_brightness(self, light: PointLight):
        """Calculate brightness for each panel using ellipsoidal falloff.
        
        V5: The diff vector is transformed by inverse rotation (Y-axis) and 
        inverse scale before computing distance. This gives an ellipsoidal
        falloff shape that can be stretched/rotated by gestures.
        
        When falloff_scale = [1,1,1] and falloff_rotation = 0, this is 
        identical to the original spherical falloff.
        """
        intensity = light.get_brightness()
        
        # Pre-compute rotation matrix (2D rotation in XZ plane around Y axis)
        rot = light.falloff_rotation
        cos_r = math.cos(-rot)  # Inverse rotation
        sin_r = math.sin(-rot)
        
        # Pre-compute inverse scale (avoid division by zero)
        inv_scale = np.array([
            1.0 / max(0.01, light.falloff_scale[0]),
            1.0 / max(0.01, light.falloff_scale[1]),
            1.0 / max(0.01, light.falloff_scale[2]),
        ])
        
        for key, panel in self.panels.items():
            diff = panel['center'] - light.position
            
            # V5: Transform diff by inverse rotation (Y-axis rotation in XZ plane)
            rotated_x = diff[0] * cos_r - diff[2] * sin_r
            rotated_z = diff[0] * sin_r + diff[2] * cos_r
            rotated_diff = np.array([rotated_x, diff[1], rotated_z])
            
            # V5: Apply inverse scale to get ellipsoidal distance
            scaled_diff = rotated_diff * inv_scale
            distance = np.linalg.norm(scaled_diff)
            
            if light.falloff_radius > 0 and distance > light.falloff_radius:
                # Outside the ellipsoidal radius — panel is off
...
def draw_ellipsoid_wireframe(center, radius, scale, rotation, color, segments=16):
    """Draw a wireframe ellipsoid (V5: anisotropic falloff visualization).
    
    Args:
        center: Position [x, y, z]
        radius: Base falloff radius
        scale: Per-axis scale factors [sx, sy, sz]
        rotation: Y-axis rotation in radians
        color: RGBA tuple
        segments: Wireframe resolution
    """
    glPushMatrix()
    glTranslatef(*center)
    glRotatef(math.degrees(rotation), 0, 1, 0)  # Y-axis rotation
    glScalef(scale[0], scale[1], scale[2])       # Per-axis stretch
    glColor4f(*color)
    glLineWidth(1)
...
def draw_auto_tuning_panel(x: int, y: int, width: int, height: int, font, font_small,
    ...
    # V6.1d: Support both V5 and V6 adjustment formats
    score = last_adjustment.get('score', 0.0)
    regime = last_adjustment.get('regime', '')
    v6_budget = last_adjustment.get('budget', 0.0)

    draw_text_2d(x + 10, y + height - 54, f"Last: {age:.0f}s ago  Score:{score:.3f}", font_small, (180, 180, 180))
    draw_text_2d(x + 10, y + height - 68, f"Regime:{regime}  Bgt:{v6_budget:.0f}", font_small, (160, 160, 160))
    ...
    # V6 SmartAutoTuner stores deltas in 'deltas' key (not 'applied_deltas')
    deltas = last_adjustment.get('deltas', last_adjustment.get('applied_deltas', {}))
    ...
    if delta_y > y + 30:
        ...
        for item in reversed(history[-max_history:]):
            ...
            # V6.1d: Support both V5 list-of-tuples and V6 dict formats
            raw_deltas = item.get('deltas', [])
            if isinstance(raw_deltas, dict):
                h_deltas = sorted(raw_deltas.items(), key=lambda kv: abs(kv[1]), reverse=True)
            else:
                h_deltas = raw_deltas
            if not h_deltas:
                continue
...
def main():
    ...
    while running and not shutdown_requested:
            ...
            try:
                ...
                state = {
                    ...
                    'light': {
                        ...
                        # V5: Anisotropic falloff shape
                        'falloff_scale': [float(s) for s in light.falloff_scale],
                        'falloff_rotation': float(light.falloff_rotation),
                    },
                    'wander_box': {
                    ...
                    'auto_tuning': {
                        'enabled': v6.autotuner._enabled if v6 and v6.autotuner else False,
                        'revision': v6._total_adjustments if v6 else 0,
                        'params': {name: round(float(getattr(meta_params, name)), 3) for name in [
                            'responsiveness', 'energy', 'attention_span', 'sociability',
                            'exploration', 'memory', 'brightness_global', 'speed_global',
                            'pulse_global', 'follow_speed_global', 'dwell_influence', 'idle_trend_weight',
                        ]},
                        'activity': None,  # V6 uses engagement score instead
                        'budget': {
                            'current': round(v6.autotuner.budget, 1) if v6 and v6.autotuner else 0,
                            'max': round(v6.autotuner.config.budget_max, 1) if v6 and v6.autotuner else 200,
                        },
                        'top_deltas': [],  # V6 reports deltas through get_state_extension
                    },
                    'population': {
        ...
        radius = 8 + brightness * 7
        draw_sphere(light.position, radius, (1, 1, brightness, 1))
        # V5: Draw ellipsoidal falloff wireframe (shows anisotropic shape)
        draw_ellipsoid_wireframe(
            light.position, light.falloff_radius,
            light.falloff_scale, light.falloff_rotation,
            (1, 0.8, 0, 0.3), segments=24
        )
        ...
        draw_text_2d(title_center_x, display[1] - 30, "LIGHT CONTROLLER V5", font)
        ...
        # V6.1d: Show V6 SmartAutoTuner state instead of disabled V5 auto_tuner
        _v6_tuner_enabled = v6.autotuner._enabled if v6 and v6.autotuner else False
        _v6_tuner_last = v6.autotuner._adjustment_history[-1] if (v6 and v6.autotuner and v6.autotuner._adjustment_history) else None
        _v6_tuner_history = v6.autotuner._adjustment_history if v6 and v6.autotuner else []
        _v6_tuner_budget = v6.autotuner.budget if v6 and v6.autotuner else 0.0
        _v6_tuner_budget_max = v6.autotuner.config.budget_max if v6 and v6.autotuner else 200.0
